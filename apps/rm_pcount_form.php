...
if($transmode == 'edit')
{
	...
	$pcount  = $functions->GetEditInfo($table,$rowid,'actual_count',$db);
	$to_shift = $functions->GetEditInfo($table,$rowid,'to_shift',$db);
...
<table style="width: 80%; top:0" class="tables" cellpadding="4" cellspacing="5">
	<tr>
		...
		<td style="width:29.5%"><input id="date" type="text" class="form-control form-input" value="<?php echo date("Y-m-d", strtotime($report_date)); ?>" disabled></td>
		<td style="width:10px"></td>
		<td style="width:29.5%">
			<input id="shift" type="text" class="form-control form-input" value="<?php echo $shift; ?>" disabled>
			<input id="toshift" type="hidden" class="form-control form-input" value="<?php echo $to_shift; ?>" disabled>
		</td>
	</tr>
</table>
<table style="width: 850px" class="tables" cellpadding="4" cellspacing="15">
	...
	<tr>
		...
		<td style="width:10px;"></td>
		<td>
			<input list="employee" id="person" class="form-control" placeholder="Select Cashier" autocomplete="no" value="<?php echo $person; ?>" onchange="set_session(this.value,'session_person')">
		...
		<td style="width:10px;"></td>
		<td><input id="slipno" type="text" class="form-control form-input" placeholder="Not Applicable" disabled></td>
	</tr>	
	<tr>
		<th>CATEGORY</th>
		<th style="width:10px;"></th>
	...
	<tr>
		<td>
			<input id="category" type="text" class="form-control" value="RAWMATS" disabled>
		<td style="width:10px;"></td>
		<td>
			<input list="items" id="itemname" class="form-control" onkeyup="itemsCheckingExist(this.value)" autocomplete="off" value="">
			<datalist id="items">
				<?php echo $dropdown->selectItemsViaCategory('RAWMATS',$db); ?>
			</datalist>
		</td>
		<td style="width:10px;"></td>
		<td>
			<input id="actual_count" type="number" class="form-control form-input" value="<?php echo $pcount; ?>" autocomplete="no">
		</td>
		<td style="width:10px;"></td>
		<td>
	...
	<tr>
		<td>
			&nbsp;&nbsp;<input style="text-align:center;color:red;font-weight:bold" type="text" id="itemid" class="form-control" value="<?php echo $item_id; ?>" placeholder="Item ID" disabled></td>
		<td style="width:10px;"></td>
		<td>
		&nbsp;</td>
		<td style="width:10px;"></td>
		<td colspan="3" style="text-align:right">
			...
			<button class="btn btn-success btnnew" onclick="commitAction('new')" disabled>
			Save Pcount</button>
			<?php } if($transmode == 'edit') { ?>
			<button class="btn btn-info btnupdate" onclick="commitAction('edit')">Update PCount</button>
			<?php } ?>
...
function itemsCheckingExist(params){
	
	var mode = 'itemsCheckingExist';	
	$.post("./actions/actions.php", { mode: mode, itemname: params },
	function(data) {
		rms_reloaderOff();
		$('.results').html(data);
	});
...
function commitAction(transmode)
{
	...
	else if(actual_count < 0 || actual_count === '' ) { app_alert("Actual Count Error","Please at least 0 if nothing is left.","warning","Ok","actual_count","focus"); return false; }
	else if(units == '') { app_alert("Units Error","Please select Unit Of Measure","warning","Ok","uom","focus"); return false; }
	if(transmode === 'new')
	{ 
		var mode = 'savermpcount'; 
