...
<table style="width: 850px" class="tables" cellpadding="4" cellspacing="15">
	...
	<tr>
		<td>
			<input id="category" type="text" class="form-control" value="RAWMATS">
		</td>
		<td style="width:10px;"></td>
		<td>
			<input list="items" id="itemname" class="form-control" placeholder="Select Items" autocomplete="no" onkeyup="getItemsRm()" value="<?php echo $item_name; ?>">
	...
	<tr>
		<td>
			...
			<p>Search term: <span id="searchTerm"></span></p>
		...
		<td colspan="3" style="text-align:right">
			<input type="text" id="itemid" value="<?php echo $item_id; ?>">
			<?php if($transmode == 'new') { ?>
			<button class="btn btn-success btnnew" onclick="commitAction('new')">Save Charges</button>
			<?php } if($transmode == 'edit') { ?>
			<button class="btn btn-info btnupdate" onclick="commitAction('edit')">Update Charges</button>
			<?php } ?>
...
function getItemsRm() {
    ...
    var mode = 'getItemsRm';
    $.post("./actions/actions.php", { mode: mode, input: inputValue }, 
    function(data) {
    	console.log(data);
        $('#items').html(data);
    });
