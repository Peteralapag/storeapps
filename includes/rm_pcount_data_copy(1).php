...
<?php
$query ="SELECT * FROM store_rm_pcount_data $q ORDER BY status,id DESC";  
...
?>	
	<tbody>	
		<tr <?php echo $tr_command; ?>>
			...
			<td style="min-width:250px;"><?php echo $ROWS['item_name']." ".$noid_text; ?></td> <!-- ITEM NAME -->
			<td style="text-align:right; padding-right:30px !important; width:150px"><?php echo $ROWS['actual_count']; ?></td> <!-- ITEM NAME -->
			<td style="width:250px;text-align:center"><?php echo $ROWS['units']; ?></td> <!-- ACTUAL YIELD -->
			...
			<td style="text-align:center; padding:1px !important" class="actions">
				<?php if($status == 'Open') { ?>
				<div>
					<button class="btn btn-success btn-sm" style="width:49%;font-size:11px" aria-label="Edit" onclick="EditUserChecking('edit','<?php echo $file_name; ?>','<?php echo $title; ?>','<?php echo $rowid; ?>')"><i class="fa-duotone fa-pencil"></i></button>
					<button class="btn btn-danger btn-sm" style="width:49%;font-size:11px" aria-label="delete" onclick="DeleteUserChecking('<?php echo $rowid; ?>','<?php echo $file_name; ?>','This Amount')" ><i class="fa-solid fa-trash"></i></button>
				</div>
				<?php }  else {?>
				<div>
